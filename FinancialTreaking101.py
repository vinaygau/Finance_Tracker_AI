...
def initialize_services():
    ...
    def init_supabase():
        try:
            ...
            if 'error' in tables:
                st.error(f"Supabase connection error: {tables['error']}")
                return None
...
def create_financial_charts(expenses_df, income_df):
    ...
    if not expenses_df.empty:
        ...
        charts['expense_by_category'] = px.pie(
            cat_data, 
            values='amount', 
            names='category', 
    ...
    if not income_df.empty:
        ...
        charts['income_by_source'] = px.bar(
            source_data, 
            x='source', 
            y='amount', 
            title="Income by Source",
            color='source',
            labels={'amount': 'Amount ($)', 'source': 'Income Source'}
        )
        
        # Income trend over time
        income_trend = income_df.groupby('date')['amount'].sum().reset_index()
        charts['income_trend'] = px.line(
            income_trend, 
            x='date', 
            y='amount', 
            title="Income Trends",
...
def get_financial_advice(user_data, query):
    ...
    try:
        prompt = f"""
        You are a professional financial advisor. Provide concise, actionable advice based on:
        ...
        Provide specific recommendations in bullet points with clear explanations.
        """
            ...
            if all(col in df.columns for col in required_columns):
                ...
                if st.session_state.supabase_connected:
                    ...
                    try:
                        ...
                        # Clear cache to refresh dynamic categories
                        st.cache_data.clear()
                    except Exception as e:
...
if current_page == "Dashboard":
    ...
    with cols[2]:
        st.markdown("<div class='metric-box'>", unsafe_allow_html=True)
        st.metric("Net Income", f"${metrics['net_income']:,.2f}")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if metrics['savings_rate'] < 10:
        st.warning("Your savings rate is low. Consider reducing discretionary spending or increasing income.")
    if metrics['expense_ratio'] > 80:
        st.warning("High expense ratio! More than 80% of your income is going to expenses.")
    if metrics['fixed_expenses'] / metrics['total_income'] > 0.5:
        st.warning("Your fixed expenses are high (>50% of income). Look for ways to reduce recurring costs.")
    ...
    if not expenses_df.empty or not income_df.empty:
        ...
        if not recent_expenses.empty:
            ...
            st.dataframe(recent_expenses[['date', 'amount', 'category', 'payment_method']])
        
        if not recent_income.empty:
            st.write("**Recent Income**")
            st.dataframe(recent_income[['date', 'amount', 'source']])
    else:
            ...
            if submitted:
                if amount <= 0:
                    ...
                    if st.session_state.supabase_connected:
                        ...
                        st.warning("Expense not saved (database not connected)")
    ...
    expenses_df = get_financial_data('expenses', current_user_id, (start_date, end_date))
    if st.session_state.mock_data and expenses_df.empty:
        expenses_df = generate_mock_data('expenses', rows=15)
    ...
    if not expenses_df.empty:
        ...
        with tab1:
            ...
            fig2 = px.bar(pm_data, x='payment_method', y='amount', 
                          title="Expenses by Payment Method", color='payment_method')
            st.plotly_chart(fig2, use_container_width=True)
        
        with tab2:
            # Daily trend
            daily_data = expenses_df.groupby('date')['amount'].sum().reset_index()
            fig3 = px.line(daily_data, x='date', y='amount', 
                          title="Daily Expense Trend", markers=True)
            ...
            fig4 = px.bar(weekly_data, x='date', y='amount', 
...
# [Additional page implementations would continue here...]
